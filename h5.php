...
<html lang="en">
  ...
  <body>
    <div class="container">
        <?php
        $tydrukud = array("mari", "kati", "maarika", "salme", "keiti", "anna", "marju", "karmen");
        sort($tydrukud);
        echo '<div class="row">';
        foreach($tydrukud as $tydruk){
            echo '<div class="col-md-3">' . $tydruk . '</div>';
        }
        ?>
        <hr>
        <?php
        ...
        $viimane_nimi = end($tydrukud);
          echo "\nViimane nimi: " . $viimane_nimi . "\n";
        ?>
        <hr>
        <?php

        $suvaline_indeks = rand(0, count($tydrukud) - 1);
        $suvaline_nimi = $tydrukud[$suvaline_indeks];
          echo "\nSuvaline nimi: " . $suvaline_nimi . "\n";
        ?>
        <hr>
        <?php
        ...
        $toyota = array_search("Toyota", $margid);
        $audi = array_search("Audi", $margid);
        echo "<br>Toyotasi on kokku: $toyota ";
        echo "<br>Audisi on kokku: $audi ";

        echo "<br> vin koodid alla 17 tahe: <br>";
        foreach ($vin as $v) {
          if (strlen($v) < 17) {
            echo $v. "<br>";
          }
